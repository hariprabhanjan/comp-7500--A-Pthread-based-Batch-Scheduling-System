/*
 *	AUbatch.c
 *	COMP7500 Operating Systems, Dr. Qin
...
static struct {
		...
		int (*func)(int nargs, char **args);
	        ...
	        /* commands: Each command must be followed by a \n.*/
	        { "?\n",        cmd_helpmenu },
		{ "h\n",        cmd_helpmenu },
		{ "help\n",     cmd_helpmenu },
		{ "r",          cmd_run },
		...
		{ "q\n",        cmd_quit },
		{ "quit\n",     cmd_quit },
		/* Add more operations if you would. */
		{ "l\n",	cmd_list },
		{ "list\n",	cmd_list },
		{ "sjf\n",	cmd_sjf },
		{ "fcfs\n",	cmd_fcfs },
...
/**************************************
 *  * The run command - submit's a job.
 * *************************************/
int cmd_run(int nargs, char **args) {
	int i = 0;
	...
	if (run) {
		printf("The Job %s", name);
		printf("was Successfully submitted.\n");
...
/*********************************************************************
 *  * The test command -tests several jobs and provides statistics.
 *   ****************************************************************/
int cmd_test(int nargs, char **args) {
	if (nargs != 7)
	{
	...
	int i;
	if (count != 0) {
		printf("\n awaiting the completion of other tasks...\n");
	...
	for (i = 0; i < jobs; i++) {
		...
		run = 1;
		//************call any arg above prints this out************
	}
	sched = 0;
...
int cmd_quit(int nargs, char **args) {
	...
	if(totalRuns < 1) {
		printf("Statistics are unavailable because not enough processes have completed.\n");
		exit(0);
	}
	printf("Total number of jobs submitted: %u\n", total);
...
int dispatch(char *cmd) {
	...
	for (word = strtok_r(cmd, " ", &context);
		...
		if (nargs >= MAXMENUARGS) {
			printf("The command line contains too many words.\n");
			return E2BIG;
		}
		args[nargs++] = word;
	}
